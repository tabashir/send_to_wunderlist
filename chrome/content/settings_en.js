...
*/
	...
	savePrefs: function() {
		...
		for( var i = 0; i < this.elementIDs.length; i++ ) {
			...
			if (eltType == "menulist" || eltType == "button") {
	...
	onLoad: function() {
		...
		for( var i = 0; i < this.elementIDs.length; i++ ) {
			...
			if (eltType == "menulist" || eltType == "button") {
				element.value = nsPreferences.copyUnicharPref(prefstr, null);
				if (!element.value) element.selectedIndex = 0;
			} else if (eltType == "label") {
				var skey = nsPreferences.copyUnicharPref(prefstr, "");
				this.setLabel(element, skey);
				element.setAttribute("skey", skey);
			}
		}
...
*/	
	editTagList: function(event) {
		...
		if (callback.isOK) {
			...
			if (pref.instantApply) {
				nsPreferences.setUnicharPref(pref.getAttribute("name"), event.target.value);
			}
		}
	},
	...
	changeKey: function(aEvent) {
		...
		if (pref.instantApply) {
			nsPreferences.setUnicharPref(pref.getAttribute("name"), promptLabel.getAttribute("skey"));
		}
